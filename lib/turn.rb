...
#Takes a users num and the board, and places the X or O in that position
def move(board, index, character = "X")
  board[index] = character
  return board
end

#Asks user for a number, check if it is valid, and if it's not, continues to ask for a number
def turn (board)
